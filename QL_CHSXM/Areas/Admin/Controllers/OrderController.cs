...
using QL_CHSXM.Models.ViewModels;
...
namespace QL_CHSXM.Areas.Admin.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        public ActionResult CreateInvoice(Order order)
        {
            if (ModelState.IsValid)
            {
                // Thực hiện lưu hóa đơn vào cơ sở dữ liệu
                db.Orders.Add(order);
                db.SaveChanges();

                // Chuyển hướng về trang index để hiển thị danh sách đơn hàng mới
                return RedirectToAction("Index");
            }

            // Nếu dữ liệu không hợp lệ, hiển thị lại form tạo hóa đơn để người dùng nhập lại
            return View(order);
        }
